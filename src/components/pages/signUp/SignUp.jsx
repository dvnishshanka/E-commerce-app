    ...
    createUserWithEmailAndPassword(auth, values.email, values.password)
      .then((userCredentials) => {
        if (userCredentials) {
          ...
          writeUserData(userCredentials.user.uid, values.name, values.email, values.address);
          // writeUserData2(items);
          navigate('/sign-in');
          messageApi.open({
            type: 'success',
      ...
      .catch((error) => {
        const errorMessage =
          error.code === FIREBASE_ERR_EMAIL_EXIST ? EMAIL_EXIST : SOMETHING_WENT_WRONG;
